...
## Intro
### Summary
This paper explores the Hexagonal architecture pattern from a different viewpoint. It intends to explain only partially the Hexagonal Architecture itself but goes beyond and explores what the Hexagonal architecture could be used for. In particular, exploring the point of view of the pluggability of business capabilities. Generally, the hexagonal architecture is described as ports or adapters to different types of services offered by software platforms, SaaS services, or infrastructure. However, if carefully designed, these ports can also be connected to other services that provide application features, not specifically technical ones. So, an application or a business component could be augmented by pluggable services that extend what an application can do without these augmentations. <br>
To set up the scenario correctly, it is important to understand the taxonomy of applications and where and how these pluggable services fit. So we will use the microservices as the starting point for this. The hexagonal architecture is usually related to the Microservices architecture. However, it is not an exclusive design for microservices architecture as the business component. But it can be any business system. The hexagonal architecture is an architectural pattern that Alistair Cockburn first detailed in 2005. At that time, the deployment options for designing these pluggable capabilities were not as advanced as today's containerisation and deployment technologies; therefore, the only alternative for implementing and deploying the ports in the hexagonal architecture was using shared libraries developed centrally but deployed multiple times in each system. <br>
The design explored in this document is an application taxonomy that opens the possibility of creating the most optimal reusable components ever designed up to this time, I think. On one side, these util capabilities can be designed in different languages, runtimes, and technology options. They are completely isolated from the business microservice and can evolve independently from any other. In addition, good versioning and deployment practices can be deployed and recycled without affecting the runtime of different components. <br>
